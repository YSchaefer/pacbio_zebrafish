...
reduced_FISNACHT_data = read.table("FISNACHT.geom_tile.heatmap_data.min_10_reads.tsv", header = T)
...
reduced_B302_data = read.table("B30.2.geom_tile.heatmap_data.min_10_reads.tbl", header = T)

ggplot(data = reduced_B302_data, aes(x = reorder(Fish, Order), y = reorder(Cluster, Relative_Depth), fill = Relative_Depth)) +
  geom_tile(show.legend = T) +
  theme_minimal() +
  scale_fill_gradient(low="white", high="black", limit = c(0,0.0001)) +
  labs(x ="Fish", y ="Cluster", fill = "Relative Depth") +
